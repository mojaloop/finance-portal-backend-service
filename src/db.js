...
SELECT totals.settlementWindowId, totals.settlementWindowStateId AS settlementWindowStateId, sum(totals.amount) AS amount, GROUP_CONCAT(totals.currencyId),
  GROUP_CONCAT(DATE_FORMAT(swOpen.createdDate, '%Y-%m-%dT%T.000Z')) AS settlementWindowOpen,
  GROUP_CONCAT(DATE_FORMAT(swClose.createdDate, '%Y-%m-%dT%T.000Z')) AS settlementWindowClose
FROM  (
