...
const handler = (router, routesContext) => {
    router.put('/logout', async (ctx, next) => {
        ...
        ctx.response.set({
            'Set-Cookie': `${ctx.constants.TOKEN_COOKIE_NAME}=deleted; expires=Thu, 01 Jan 1970 00:00:00 GMT;HttpOnly; SameSite=strict${routesContext.config.insecureCookie ? '' : '; Secure'}`,
        });
        ctx.response.body = {
            status: 'Ok',
        };
